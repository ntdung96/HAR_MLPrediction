...
### a. Model 1: Linear discriminant analysis (LDA)
...
The accuracy of LDA model is `r model_lda$results$accuracy`.

### b. Model 2: Random Forest

```{r}
#Fit Random Forest with total 400 trees grown and 7 variables each split
model_rf <- train(classe ~ ., method = "rf", ntree=400,
                  tuneGrid=data.frame(.mtry = 7),
                  data = train)
model_rf
```
The accuracy of Random Forest model is `r model_rf$results$accuracy`.
...
ggplot2::qplot(pred_lda, pred_rf, color = test$classe)
```
